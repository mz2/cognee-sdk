# Contributing to the cognee SDK
...
- `services/` — the systemd user unit for the optional REST API server.
- `VERSION` — the upstream cognee release this branch tracks (version branches
  only; `main` carries no `VERSION`).
...
workshop launch                   # using examples/workshop.yaml (try-cognee)
